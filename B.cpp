...
Big_Int pow(Big_Int, Big_Int, Big_Int); // Возведение в степень
...
// Возведение в степень:
Big_Int pow(Big_Int a, Big_Int n, Big_Int p) {
    Big_Int res = 1;
    while (n > 0) {
        if (n % 2 != 0) res *= a;
        a *= a;
...
int main() {
    ...
    while (cin >> a >> b) {
        Big_Int A(a);
        Big_Int B(b);
        Big_Int res = B * (pow(A, p - k - 1, p)) % p;
        num_in_10 += cnt * res;
        cnt *= p;
    }
    while (num_in_10 != 0) {
        cout << number_to_char(num_in_10 % 64);
        num_in_10 /= 64;
    }
}
